...
//        writeSourceFile = true;
        sep(c -> {
            FlowSupplier<HouseDetails> processedDouseDetails = DataFlow.subscribe(HouseDetails.class).flowSupplier();
            c.addNode(new PredictiveLinearRegressionModel(PropertyToFeature.build("area", processedDouseDetails, HouseDetails::getArea)), "predictiveModel");
        });
...
//        writeSourceFile = true;
        sep(c -> {
            FlowSupplier<HouseDetails> processedDouseDetails = DataFlow.subscribe(HouseDetails.class).flowSupplier();
            c.addNode(new PredictiveLinearRegressionModel(PropertyToFeature.build("area", processedDouseDetails, HouseDetails::getArea, RegressionTest::doubleArea)), "predictiveModel");
        });
        ...
        //send record to generate a prediction
        onEvent(new HouseDetails(12, 3));
        Assert.assertEquals(72, predictiveModel.predictedValue(), 0.000_1);
    }

    public static double doubleArea(double input) {
        return input * 2;
    }

...
import com.fluxtion.creator.MathFactory.Value;
...
/*
 * <pre>
 * generation time   : 2021-12-06T11:01:01.086534800
...
public class TestSep_factoryMethod implements StaticEventProcessor, BatchHandler, Lifecycle {
  ...
  public void handleEvent(MyPredefinedEvent typedEvent) {
    ...
    if (isDirty_dataHandler) {
      ...
      isDirty_myProcessor = true;
      myProcessor.process();
    }
    if (isDirty_myProcessor) {
      auditInvocation(max, "max", "evauateMax", typedEvent);
      max.evauateMax();
    }
  ...
  private void initialiseAuditor(Auditor auditor) {
    auditor.init();
    auditor.nodeRegistered(max, "max");

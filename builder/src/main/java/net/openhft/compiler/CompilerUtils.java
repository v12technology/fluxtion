 ...
 * distributed under the License is distributed on an "AS IS" BASíIS,
...
public enum CompilerUtils {
    ...
    private final static Logger LOGGER = Logger.getLogger(CompilerUtils.class.getName());
    ...
    public static void defineClass(@NotNull String className, @NotNull byte[] bytes) {
        defineClass(new MyClassLoader(Thread.currentThread().getContextClassLoader()), className, bytes);
    }
     ...
     * @param bytes       of the byte code.
     */
    public static Class defineClass(@Nullable MyClassLoader classLoader, @NotNull String className, @NotNull byte[] bytes) {
        return classLoader.defineClassOverride(className, bytes, 0, bytes.length);
    }
    ...
    private static byte[] readBytes(@NotNull File file) {
        ...
        try {
            ...
            close(dis);
            LOGGER.log(Level.WARNING, "Unable to read {0} error:{1}", new Object[]{file, e});
        ...
        if (closeable != null)
            try {
                closeable.close();
                 ...
                 LOGGER.log(Level.WARNING, "Failed to close {0} error:{1}", new Object[]{closeable, e});
            }
    }
    ...
    public static boolean writeBytes(@NotNull File file, @NotNull byte[] bytes) {
        ...
        if (file.exists()) {
            ...
            if(file.renameTo(bak)){
                System.out.println("renamed to:" + bak);
            }else{
        ...
        try {
            ...
            close(fos);
            LOGGER.log(Level.WARNING, "Failed to write {0} as:{1} error{2}", new Object[]{file, decodeUTF8(bytes), e});
            file.delete();
            if (bak != null)
                bak.renameTo(file);
